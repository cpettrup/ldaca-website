...
const getGridClass = (items) => {
  return items.length > 3 ? 'sm:grid-cols-2' : 'sm:grid-cols-3'
}
</script>
...
<template>
  <header class="w-full border-border relative">
    ...
    <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 -translate-y-1"
      ...
      <div v-if="activeMenu"
        class="hidden md:block absolute left-0 right-0 border-b border-border shadow-lg z-50 bg-[#FFFEF8]" :style="{
        ...
        <div class="container mx-auto py-6 px-6">
          <div v-for="item in nav" :key="item.text" v-show="activeMenu === item.text">
              ...
              <div :class="[
                ...
                <template v-for="(subItem, idx) in item.items" :key="`${subItem.text || subItem.title || 'item'}-${subItem.link || idx}`">
                  ...
                  <div v-else class="flex flex-col"
                    :class="(subItem.divider && idx % 3 !== 2) ? 'sm:border-r border-border pr-4' : ''">
    ...
    <div v-if="activeMenu" @click="closeMenu" class="hidden lg:block fixed inset-0 z-40" />
    ...
    <transition enter-active-class="transition ease-out duration-300" enter-from-class="opacity-0"
      ...
      <div v-if="mobileMenuOpen" @click="closeMobileMenu" class="md:hidden fixed inset-0 bg-black bg-opacity-50 z-40" />
    </transition>
  </header>
